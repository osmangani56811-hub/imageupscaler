...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .item .status.working { background: rgba(232,163,61,0.15); color: var(--amber); display: flex; align-items: center; gap: 5px; }
  .spinner {
...
function renderQueue() {
  ...
  for (const item of queue) {
    ...
    const retryBtn = item.status === 'error' ? '<button class="retry-btn" data-retry="' + item.id + '">Retry</button>' : '';
    div.innerHTML =
      '<img src="' + url + '">' +
  ...
  document.querySelectorAll('[data-retry]').forEach(btn => {
    btn.addEventListener('click', () => retryOne(btn.getAttribute('data-retry')));
  });
}
...
processBtn.addEventListener('click', async () => {
  ...
  const target = document.getElementById('targetSelect').value;
  const mode = document.getElementById('modeSelect').value;
  const CONCURRENCY = 1; // one image at a time, as requested — most reliable, clearest progress
